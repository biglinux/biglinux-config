...
def get_installed_flatpaks() -> list[AppEntry]:
    ...
    for line in result.stdout.strip().splitlines():
        ...
        base = os.path.expanduser(f"~/.var/app/{app_id}")
        if not os.path.isdir(base):
            continue

        # Prefer the concrete config/ and data/ subtrees and deliberately skip
        # cache/ (it is volatile and regenerated by the app).  Fall back to the
        # whole per-app directory only when neither subtree exists.
